...
mnlMetropOnceOut mnlMetropOnce_conLogNormal(vec const& y, mat const& X, vec const& oldbeta, 
  ...
  int stay = 0;
  vec betac = oldbeta + exp(s*trans(incroot)*as<vec>(rnorm(X.n_cols)));
// s = 0.1;
//   vec betac = exp(log(oldbeta) + s*trans(incroot)*as<vec>(rnorm(X.n_cols)));
...
// Wayne Taylor 10/01/2014
  ...
  // for log normal prior, have to initialize beta at a positive value
  betadraw.fill(0.1);
  oldbetas.fill(0.1);
  ...
  for (int rep = 0; rep<R; rep++){
    ...
    // when sampling delta and V_beta, regress log(beta) on ZDelta
    List mgout;
    if(drawdelta) {
    ...
    if(drawdelta) olddelta = drawDelta(Z,log(oldbetas),ind,oldcomp,deltabar,Ad);
...
// mnlMetropOnceOut mnlMetropOnce_con_LogNormal(vec const &y, mat const &X, vec const &oldbeta,
//                                              double oldll, double s, mat const &incroot,
//                                              vec const &betabar, mat const &rootpi, vec const &SignRes = NumericVector::create(2))
// {
//     // Wayne Taylor 10/01/2014
...
//     // use lndLogMvn instead of Normal
...
//         if (drawdelta)
//         {
//             // when drawing delta, pass log(beta) ~ delta * Z
//             olddelta.reshape(nvar, nz);
//             mgout = rmixGibbs(log(oldbetas) - Z * trans(olddelta), mubar, Amu, nu, V, a, oldprob, ind);
//         }
//         else
...
//             metropout_struct = mnlMetropOnce_con_LogNormal(lgtdata_vector[lgt].y, lgtdata_vector[lgt].X, vectorise(oldbetas(lgt, span::all)),
//                                                            oldll[lgt], s, incroot, betabar, rootpi, SignRes);
