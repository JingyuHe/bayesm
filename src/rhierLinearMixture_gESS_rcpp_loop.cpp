...
unireg gESS_draw_hierLinearModel(vec const &y, mat const &X, vec const &beta_ini, vec const &beta_hat, mat const &L, double tau)
{
    ...
    */
    ...
    double priorcomp = log_likelihood_reg3(y, X, beta_ini, tau);
    ...
    compll = log_likelihood_reg3(y, X, betaprop + beta_hat, tau);
                                        ...
                                        mat olddelta, vec const &a, vec oldprob, vec ind, vec tau)
{
    ...
    for (int rep = 0; rep < R; rep++)
    {
        ...
        for (int reg = 0; reg < nreg; reg++)
        {
            ...
            runiregout_struct = gESS_draw_hierLinearModel(regdata_vector[reg].y, regdata_vector[reg].X, trans(oldbetas(reg, span::all)), betabar, L, tau[reg]);
